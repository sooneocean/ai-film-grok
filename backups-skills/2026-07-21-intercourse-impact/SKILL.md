---
name: ai-film-grok
description: Grok Build 专用 AI 短片 skill：八环 Idea→Verified 自动调配（dispatch）+ Imagine 静帧/I2V bulk（grok_primary，Seedance 暂关）+ OAuth + edge TTS + HF/FFmpeg。触发：AI 电影、漫剧、Grok Imagine、dispatch、成片、/ai-film-grok。
---
...
**工具栈（Grok Build 最大化 · 当前 I2V=`grok_primary`）**

| 层 | Grok Build 原生 | 本 skill 本地/外部 |
|----|-----------------|-------------------|
| 脑 | 推理 · Structured 输出 · 长上下文 | film-spec / Lens / dispatch 决策 |
| 搜 | `web_search` · `x_*` | 事实接地后再出 Prompt |
| 静帧 | **`image_gen` / `image_edit`**（加载 `/imagine`） | lock-style · cast master |
| 动态 | **`image_to_video` bulk（Seedance 暂关）** | queue/register `image_to_video`；FRW 仅可选 env |
| 语音 | 会话≠成片音轨 | **edge** / Voicebox / CosyVoice |
| 记忆 | 工作区文件 | film-root + receipts |
| 成片 | — | FFmpeg plate · HyperFrames · review |

**I2V 运营模式**：`AIFILM_I2V_PROFILE=grok_primary`（默认）→ `write-spec` 时 `auto` 钉 **`i2v_provider=grok`**。见 [i2v-grok-primary.md](references/i2v-grok-primary.md)。  
Seedance 恢复：`AIFILM_I2V_PROFILE=seedance_first` + canary 201。  
**FRW**：环境床可选 `ltx-t2v`；**禁**默认 legacy img2video。  
**Grok OAuth**：`aifilm grok-oauth doctor`。出图前加载 **`/imagine`**。
...
"$AIFILM" doctor
"$AIFILM" grok-oauth doctor           # OAuth 绿：chat/imagine API 可用
...
中文 **edge 默认**（零依赖、可复现）。**质量档**（不改默认）：本机克隆 → `voicebox`；中文最高自然度本地 → **CosyVoice 2** `external`（`adapters/cosyvoice_tts.py` + 固定 ref）；情感精修 → MiniMax / Higgs 候选。场景表见 [voices.md](references/voices.md) · 开源横评对照 [opensource-tts.md](references/opensource-tts.md)。**失败 opt-in 兜底**：`AIFILM_TTS_VOICEBOX_FALLBACK=1`（不静默换声）。有回执则 vo_pacing **优先 measured**。混音在 final plate（sidechain / loudnorm / auto_sfx）。
...
| **Grok OAuth**（grok login） | [grok-oauth.md](references/grok-oauth.md) |
