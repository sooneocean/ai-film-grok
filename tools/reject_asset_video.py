#!/usr/bin/env python3
"""Reject a pending_human_review video asset: route it out of the pipeline.

Terminal transition pending_human_review -> rejected. The media file is moved
to rejected/ so pending/ stays clean and the review queue cannot re-show it.
Mirrors tools/reject_asset.py for the video lane.

    python3 tools/reject_asset_video.py --asset-id t2v-cinematic-abc12345 \\
        --reviewer dex --reason "too dark / frozen"
    python3 tools/reject_asset_video.py --asset-id ... --dry-run
"""
import argparse
import os
import shutil
...
from video_pipeline_lib import (load_vcatalog, save_vcatalog, VIDEO_LIB, now_iso,
                                can_transition)
...
def main():
    ...
    ext = src.rsplit(".", 1)[-1] if os.path.exists(src) else "mp4"
    dst = os.path.join(VIDEO_LIB, f"rejected/{args.asset_id}.{ext}")
    ...
    if os.path.exists(src):
        os.makedirs(os.path.join(VIDEO_LIB, "rejected"), exist_ok=True)
        shutil.move(src, dst)
    ...
    if ok.returncode != 0:
        print("ERROR: video catalog validation failed after reject; restoring backup",
              file=sys.stderr)
