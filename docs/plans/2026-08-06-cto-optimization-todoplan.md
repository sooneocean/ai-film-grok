...
| **`docs/plans/2026-08-07-delivery-honesty-rail-todoplan.md`** | **出片诚实审计轨** · Active 2.40.68 R0–R3 SHIPPED (`skip_audit`+attestation+checkout_drift)；R4 I5 residual |
...
| ID | Todo | 节奏 | 验收 |
|----|------|------|------|
| **C6.1** | **低 importer 优先迁** | 每周 **5–10** 模块 → package + 薄 shim | metabolism inventory 更新 |
| **C6.2** | **优先厚顶层** | `workflow_pack` / `input_fidelity` / `state_index_gate` / `prompt_injector` / `shortform_director`… | 归属 spine/gates/plan |
| **C6.3** | **Lane A 删除** | 0 import ∧ 0 CLI ∧ 0 test → 删 | 每 batch 报告 |
| **C6.4** | **基座测试补漏** | `core/*` `util/*` 公共 API | P4 延续；每模块 ≥ 契约测 |
