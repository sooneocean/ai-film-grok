...
| 本地 check-all ≡ CI（secret+hotpath） | **DONE** | 2.39.92 |
...
| media_queue job-level backoff 单一函数 `scheduled_backoff_sec` | **DONE**（非 process sleep） | 2.39.95 |
...
| 虚荣 peel / 全仓 media_queue process-retry 重写 | **NON-GOAL** | — |
...
| hotpath 可选 | `make test-hotpath` / CI job / `make review` |
