...
| **H3 Fill-Idle primary 分类纯函数化（P0a/P0b/P0c + dual leg）** | ✅ **本会话** | v2.40.19 `classify_primary_h3_shot` 抽离为纯函数+单测（`test_fill_idle_primary_classify.py` 11 用例）；把 `classify_fill_idle_shot` 的 `elif primary:` 块（P0a/P0b/P0c 分级 + dual I2V+R2V 第二腿含 γ3 强身份腿跳过盲 R2V）提升为可测不变量；`classify_fill_idle_shot` 调用并 extend reasons，输出完全一致。闭合 v2.40.15–17 Fill-Idle 自动派单分解 | 
