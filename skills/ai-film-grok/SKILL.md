...
7. **动作供应商**：`grok_primary` 云主链：**Grok I2V → FRW API I2V → FRW LTX 2.3**（对白锁 LTX 有声）。**`hybrid_h3`**：bulk 仍 Grok；restricted/肉戏 soft-lock **本地 MiniMax H3**（`comfy-h3` pilot，禁静默 bulk；`aifilm h3 plan|run`）。禁 Seedance 与 Wan 2.2 本地 I2V。H3 **原声可用则保留**（`prefer_native`）；仅不可用或显式 `strip_native_use_tts_bgm` 才走 Edge TTS/rnb。
