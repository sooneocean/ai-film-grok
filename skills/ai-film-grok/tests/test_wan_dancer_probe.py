...
from wan_dancer_probe import REQUIRED_CLASS_TYPES, REQUIRED_MODELS, probe_wan_dancer  # noqa: E402


def _object_info(*, complete: bool = True) -> dict[str, object]:
    classes = {class_type: {} for class_type in REQUIRED_CLASS_TYPES}
    if not complete:
        classes.pop("WanDancerEncodeAudio")
    return classes


@patch("wan_dancer_probe._json_request")
def test_probe_proves_named_dependencies_without_claiming_execution_ready(
    ...
    report = probe_wan_dancer("http://127.0.0.1:18188")
...
@patch("wan_dancer_probe._json_request")
def test_probe_fails_closed_when_the_local_dancer_model_is_missing(
    request: MagicMock,
) -> None:
    request.side_effect = lambda _url, route: {
        "/object_info": _object_info(),
        "/models/diffusion_models": [REQUIRED_MODELS["diffusion_models"][0]],
        "/models/audio_encoders": [REQUIRED_MODELS["audio_encoders"]],
    }[route]
    ...
    assert report["missing_weights"] == {
        "diffusion_models": [REQUIRED_MODELS["diffusion_models"][1]]
    }
    assert report["execution_ready"] is False


@patch("wan_dancer_probe._json_request")
def test_probe_fails_closed_when_audio_conditioning_node_is_missing(
    request: MagicMock,
) -> None:
    request.side_effect = lambda _url, route: {
    ...
    assert report["missing_class_types"] == ["WanDancerEncodeAudio"]
    assert report["execution_ready"] is False


def test_registry_keeps_wan_dancer_research_only_and_non_promotable() -> None:
    weapon = next(
        item
        for item in load_armory()["research_weapons"]
        if item["id"] == "wan22-dancer-performance-research"
    )
