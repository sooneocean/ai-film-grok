...
class TTSAbTests(unittest.TestCase):
    ...
    def test_skip_voicebox_when_down_edge_ok(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_synth(text, out_mp3, **kwargs):
                ...
                return {
                    "backend": kwargs.get("backend") or "edge",
                    "path": str(p),
                    "voice": "zh-CN-XiaoxiaoNeural",
                }

            with (
                mock.patch("tts_backend.probe") as pr,
                mock.patch("tts_backend.synthesize", side_effect=fake_synth),
            ):
                pr.return_value = {
                    "ok": True,
                ...
                with mock.patch.dict("sys.modules"):
                    ...
                    with (
                        mock.patch.object(tb, "probe", pr),
                        mock.patch.object(tb, "synthesize", side_effect=fake_synth),
                    ):
                        man = run_tts_ab(
                            root,
                            shot_id="shot01",
    ...
    def test_missing_shot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch(
                "tts_backend.probe",
                return_value={"backends": {"edge": True}, "ready": {"edge": True}},
            ):
                import tts_backend as tb
