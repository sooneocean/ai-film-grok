...
def _valid(root: Path, *, duplicate: bool = False) -> None:
    _write(root, "series-bible.json", {"series_id": "night", "title": "夜航", "season_arc": "信任变质", "release_cadence": "每周两集", "mature_content": {"adult_only": True, "explicit_consent": True}, "characters": [{"id": "a", "adult_confirmed": True, "relationship": "搭档", "motivation": "寻找真相", "contrast": "冷静外表下冲动", "rights": {"source_type": "original"}}], "episodes": [{"episode_id": "ep01", "episode_number": 1, "novelty_signature": "office-secret"}, {"episode_id": "ep02", "episode_number": 2, "responds_to_hook_id": "ep01-ending", "novelty_signature": "office-secret" if duplicate else "museum-alibi"}]})
    _write(root, "drama-graph.json", {"beats": [{"id": "b1", "event_relation": "introduces"}, {"id": "b2", "event_relation": "complicates"}]})
    _write(root, "film-spec.json", {"serial": {"enabled": True, "series_id": "night"}, "episode_contract": {"episode_id": "ep02", "primary_event": "交换证据", "event_turn": "证据被调包", "visible_outcome": "主角发现标记", "opening_hook": "门外有人", "ending_question": "谁调包了证据", "expected_next_payoff": "追查标记", "title": "证据被调包", "synopsis": "搭档在办公室交换证据时发现真相", "conflict_basis": "证据被调包", "relationship_basis": "搭档互不信任", "opening_promise": {"characters": "两位搭档", "setting": "办公室", "conflict": "证据被调包", "evidence_shot_ids": ["s1"]}, "novelty": {"genre_tags": ["悬疑"], "setting": "办公室", "differentiator": "证据交换", "signature": "office-secret"}}, "scenes": [{"shots": [{"id": "s1", "duration_sec": 12}, {"id": "s2", "duration_sec": 12}]}]})
