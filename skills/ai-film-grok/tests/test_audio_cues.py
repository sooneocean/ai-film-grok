...
def test_voice_text_is_separate_from_foley_and_compiles_to_absolute_time():
    shot = _shot([
        {"kind": "voice", "speaker": "heroine", "line_type": "dialogue", "spoken_text": "别回头。", "emotion": "fear", "performance": {"rate": 0.8}, "start_offset_sec": 1.2, "duration_sec": 1.4},
        {"kind": "foley", "asset_hint": "wet_footsteps", "start_offset_sec": 0.1, "duration_sec": 0.8},
        {"kind": "silence", "purpose": "hear the door", "start_offset_sec": 3.0, "duration_sec": 0.6},
    ])
...
def test_non_voice_text_never_enters_tts_contract():
    with pytest.raises(AudioCueError, match="only allowed on voice"):
        validate_audio_cues([_shot([{"kind": "sfx", "spoken_text": "脚步声", "asset_hint": "footsteps", "start_offset_sec": 0, "duration_sec": 1}])], strict=True)
...
def test_strict_mode_never_falls_back_to_legacy_narration_or_stage_directions():
    ...
    with pytest.raises(AudioCueError, match="sound/action direction"):
        validate_audio_cues([_shot([{"kind": "voice", "speaker": "narrator", "line_type": "narration", "spoken_text": "（脚步声）快走", "start_offset_sec": 0, "duration_sec": 1}])], strict=True)
...
def test_voice_must_fit_its_shot_and_only_one_turn_is_renderable():
    with pytest.raises(AudioCueError, match="exceeds"):
        validate_audio_cues([_shot([{"kind": "voice", "speaker": "narrator", "line_type": "narration", "spoken_text": "一段话", "start_offset_sec": 4.5, "duration_sec": 1}])], strict=True)
    shot = _shot([
        {"kind": "voice", "speaker": "a", "line_type": "dialogue", "spoken_text": "甲", "start_offset_sec": 0, "duration_sec": 1},
        {"kind": "voice", "speaker": "b", "line_type": "dialogue", "spoken_text": "乙", "start_offset_sec": 2, "duration_sec": 1},
    ])
