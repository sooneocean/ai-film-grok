...
| 主题 | 文件 | 要点 |
|------|------|------|
| **2026-08-04 全面优化收工** | [session-wrap-optimize](2026-08-04-session-wrap-optimize.md) · Fill-Idle 2.37.5 + meaning 2.37.6 | check-all 绿；push 齐 |
...
| **I2V 静帧–prompt 对齐 + interrupt 假进度 IRON（P0 · 后面不要再犯）** | [i2v-still-prompt-match-comfy-interrupt](2026-07-29-i2v-still-prompt-match-comfy-interrupt.md) · [lesson](../references/lessons-2026-07-29-i2v-still-prompt-match-comfy-interrupt.md) | still=prompt；mean≠质量；interrupt=0 进度；抢卡 PARTIAL |
...
- `AI FILM SPACE/0728/btc-vessel-ep02-power-seed`：motion gate **13/14**；sc11 静帧=机房站姿却套 afterglow prompt → mean~12 质量差；重渲 interrupt 空转；**PARTIAL**（见 still–prompt 对齐 IRON）
