...
def _probe_media(clip: Path) -> dict[str, Any]:
    ...
    if not all((fps > 0, duration > 0, int(stream.get("width") or 0) > 0, int(stream.get("height") or 0) > 0)):
        raise VisualTextAuditError("input video has incomplete geometry or frame rate")
    return {"fps": fps, "duration_sec": duration, "width": int(stream["width"]), "height": int(stream["height"])}
...
def _extract_all_frames(clip: Path, destination: Path) -> list[Path]:
    ...
    try:
        subprocess.run(
            ["ffmpeg", "-y", "-i", str(clip), "-map", "0:v:0", "-vsync", "0", str(destination / "frame_%08d.png")],
...
def _review_batch(base_url: str, model: str, token: str | None, frames: list[Path]) -> list[dict[str, Any]]:
    ...
    try:
        response = _request_json(normalize_base_url(base_url), "/chat/completions", token=token, timeout=120, body={
            "model": model, "temperature": 0, "max_tokens": 700,
            "response_format": {"type": "json_schema", "json_schema": {"name": "visual_text_audit", "strict": True, "schema": _RESPONSE_SCHEMA}},
            "messages": [{"role": "system", "content": "Return only strict JSON."}, {"role": "user", "content": content}],
        })
...
def audit_clip(
    root: Path | str, clip: Path | str, *, base_url: str, model: str, token: str | None = None,
    review_batch: Callable[[str, str, str | None, list[Path]], list[dict[str, Any]]] = _review_batch,
) -> dict[str, Any]:
    ...
    try:
        ...
        raise VisualTextAuditError("visual text clip must be a regular file inside the film workspace") from exc
    ...
    frame_records = [{"index": index, "path": str(frame.relative_to(root_path)), "sha256": sha256_file(frame), "timestamp_sec": round(index / media["fps"], 6)} for index, frame in enumerate(frames)]
    ...
    for start in range(0, len(frames), BATCH_SIZE):
        ...
        for issue in issues:
            ...
            findings.append({**record, "location": str(issue["location"]), "confidence": float(issue["confidence"])})
    if sha256_file(source) != source_sha or any(sha256_file(Path(item["path"]) if Path(item["path"]).is_absolute() else root_path / item["path"]) != item["sha256"] for item in frame_records):
        raise VisualTextAuditError("input video or extracted frames changed during audit")
    report = {
        "schema_version": SCHEMA_VERSION, "kind": "visual-text-audit", "at": _now(),
        "status": "rejected" if findings else "clean", "reason": "PROVIDER_VISUAL_TEXT_REJECTED" if findings else None,
        "clip": {"path": str(source.relative_to(root_path)), "sha256": source_sha, **media},
        "sampling": {"mode": "every_decoded_frame", "frame_count": len(frame_records), "batch_size": BATCH_SIZE},
        "reviewer": {"base_url": normalize_base_url(base_url), "model": model},
        "frames": frame_records, "findings": findings, "human_review_required": True,
    }
...
def require_clean_audit(root: Path | str, clip: Path | str, receipt: Path | str | None = None) -> dict[str, Any]:
    root_path = Path(root).expanduser().resolve()
    source = safe_existing_file(root_path, Path(clip).expanduser(), field="visual text clip")
