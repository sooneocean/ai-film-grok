...
def run_preflight(root: Path) -> dict[str, Any]:
    ...
    # Premium vertical (W3 leaf)
    from gates.preflight_premium import append_premium_vertical_issues

    append_premium_vertical_issues(root, hard, soft)
...
def _apply_preflight_tts_and_heat(
    root: Path, spec: dict, hard: list[dict], soft: list[dict], score: dict | None
) -> tuple[dict[str, Any] | None, dict | None]:
    """VO / TTS rehearsal timing + heat / sex-floor / stance / BGM / motion /
    frame-chain / anti-boring / face-identity checks (was run_preflight `if spec:` body, A).
    Zero-drift mechanical extraction; returns the possibly-rebound tts_timing and score.
    """
    tts_timing: dict[str, Any] | None = None
    ...
    return tts_timing, score


def _apply_preflight_art_and_edit(
    root: Path, spec: dict, style: dict, hard: list[dict], soft: list[dict]
) -> None:
    """Headroom / transition / read-back / style-bible / speaker / dialogue /
    dramatic / production / composition / contract / scene-art checks (was run_preflight
    `if spec:` body, B). Zero-drift mechanical extraction; mutates hard/soft in place.
    """
    ...
    try:
        ...
        if tp_codes:
            ...
            if tp_soft and not continue_hard:  # noqa: SIM108 (kept byte-identical to HEAD; ternary would drift this block)
                severity = "soft"
            else:
