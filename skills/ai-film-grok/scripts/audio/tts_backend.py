...
def cosyvoice_local_argv_configured() -> bool:
    ...
    if not argv or len(argv) < 2:
        return False
    adapter = (Path(__file__).resolve().parent.parent / "adapters" / "cosyvoice_local_tts.py").resolve()
    try:
...
def kokoro_local_argv_configured() -> bool:
    ...
    if not argv or len(argv) < 2:
        return False
    adapter = (Path(__file__).resolve().parent.parent / "adapters" / "kokoro_tts.py").resolve()
    try:
        configured = Path(argv[1]).expanduser().resolve()
    except OSError:
        return False
...
def _local_adapter(
    ...
    *,
    ...
    script = Path(__file__).resolve().parent.parent / "adapters" / f"{name}.py"
