#!/usr/bin/env python3
"""非生产 · research-only readiness probe for Wan 2.2 Dancer (not film spine)."""
...
def probe_wan_dancer(base_url: str) -> dict[str, Any]:
    ...
    if not isinstance(object_info, Mapping):
        raise RuntimeError("Wan Dancer probe received invalid ComfyUI object_info")
    ...
    return {
        "schema_version": 1,
        "kind": "wan22-dancer-readiness-probe",
        "ok": True,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Probe Wan 2.2 Dancer dependencies without loading, downloading, or submitting."
    )
    parser.add_argument(
        "--base-url",
        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = probe_wan_dancer(args.base_url or default_base_url())
