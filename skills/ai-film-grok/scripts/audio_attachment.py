...
def bind(
    ...
    approval = root / str(cue["approval_receipt"]).removeprefix("local:")
    ...
    record = {
        ...
        "approval_receipt": str(approval.relative_to(root)),
...
def valid(root: Path, event: dict[str, Any]) -> bool:
    ...
    try:
        ...
        approval = root / str(event.get("approval_receipt") or "").removeprefix("local:")
        cue = {
            key: event.get(key)
            for key in (
                "duration_sec",
        ...
        return bool(
            ...
            and _confined_without_symlinks(root, approval, require_file=True)
            and record.get("approval_receipt") == str(approval.relative_to(root))
