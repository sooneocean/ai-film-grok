...
| **I2V 末帧不回穿 + promote 门（强化 I2.2 · 2.40.51）** | **P0**：`register-clip approved` 对 undress/meat 跑 `endframe_wardrobe` 启发式（首/末帧 skin ratio；`ENDFRAME_REDRESS_RISK` hard）。毒末帧禁 promote；identity 只锁脸发。逃生 `AIFILM_SKIP_ENDFRAME_WARDROBE=1`。见 [i2v-endframe-no-redress](lessons-2026-07-22-i2v-endframe-no-redress.md) |
...
| **H3 官方 prompt 方言 · auto（P0 · 2026-08-07 · O3+optimize）** | 编译器 `scripts/media/h3_official_prompt.py`（vendor `references/vendor/minimax-h3/` · HF base/ref GUIDE）。**默认 `auto`**：对白→official（`<d>[Mandarin]` + 三字段/Ref2VA）；**high→legacy** 时间轴（O3 mean）；其余 official。T2VA 禁 Picture1；official 禁 2V stage / legacy 尾缀；`validate_official_prompt` fail-closed（`AIFILM_H3_OFFICIAL_SOFT=1` 逃）。强制：`AIFILM_H3_PROMPT_DIALECT=official\|legacy`。证据 `artifacts/2026-08-07-h3-official-ab-canary.json` · [optimize todo](../../docs/plans/2026-08-07-h3-official-prompt-optimize-todoplan.md)。 |
