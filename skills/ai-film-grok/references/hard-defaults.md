...
| **Gate-auto 机读过闸（P0 · 2026-08-04）** | **禁止** agent 反复让用户手点闸。`aifilm gate-auto --root` 自动：mean 测量 → 写 i2v-final-gate → sex_sfx inject → five_track → 单 take promote → true_video → variety → cinematic-gate → `receipts/gate-auto.json`。closeout 在 cinematic 红时自动跑。**永不**自动绿：pilot 用户批、多 take 人 PK、review-final 计分卡、付费预算 ack。逃生 `AIFILM_SKIP_GATE_AUTO=1`。代码：`scripts/gate_auto.py`。 |
