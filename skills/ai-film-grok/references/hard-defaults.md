...
| **plate 有片仍无聊 + mix 假死（P0 · 2026-08-06 savani）** | **有 plate ≠ 好看**。肉戏 mean 大面积≪20 装片 = 静帧联播；**variety 只改 film-spec 字段不算交差**，须 re-I2V/promote 后像素变。final 混音：**禁** 默认硬等 `sidechain+acrossover` dynamic_eq（可假死 30–60m+ 不写 `mixed.wav`）；优先 **`AIFILM_FORCE_BROADBAND_DUCK=1`**（轻 duck），再不行 **`AIFILM_FORCE_SIMPLE_AMIX=1`**（PARTIAL 诚实）。分辨率变体 take **不算**创意 multi-take。交付前抽：mean 审计 + 邻镜肉眼差 + 对白时 BGM 有无起伏。见 [memory](../memory/2026-08-06-plate-boring-mean-mix-iron.md) · [anti-boring](lessons-2026-07-29-shot-variety-anti-boring.md) · 高动态常态 |
