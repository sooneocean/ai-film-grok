...
| **Comfy 隧道端口与排队 IRON（P0 · 2026-07-29 · 后面不要再犯）** | 本机 **`18188` 只转到远程 Comfy `8188`**。`18188→8189` = 鉴权服务 → `{"detail":"unauthorized"}` 401（**不是** Comfy 要登录）。健康探针：`/system_stats` 200 + Comfy JSON。lipsync 用 **18790→8790**。bulk 占卡：idle **立刻** `run-workflow`，禁先 free-memory 丢空档；禁盲 cancel 未知 running。见 [tunnel-8188-not-8189](lessons-2026-07-29-comfy-tunnel-8188-not-8189.md) · [memory](../memory/2026-07-29-comfy-tunnel-queue-neon-canary.md) |
...
| **bulk→final 出片 IRON（P0 · 2026-07-29 · 后面不要再犯）** | **Imagine 拦 bare I2V** → undress 末帧 continue + HIGH MOTION 软词 + MEDIUM LOCK cel；禁 photoreal assembly；soft-pass 须测 mean 再装片；禁内衣冒充插入。**重拍后**全轨 review+register **两轮**（邻接 quality_evidence sha）。orphan pilot 清出 `manifest.clips`。max 须 `undress-anchor` + `cast-states/<id>/{undressed,bare}.png`。**final**：`aifilm final` 1200s 易假死 → 直调 `render_final` ≥1800s；sidechain 混音卡住 → 简化 amix；`--music` 须 license；字幕路径空格/force_style 炸 → `/tmp` 或 PIL 硬烧。见 [evirus-ch04-bulk-final-iron](lessons-2026-07-29-evirus-ch04-bulk-final-iron.md) |
...
| 规则 | 默认 |
|---|---|
| bulk 动作 I2V | **双车道默认（成人 max）**：setup/非敏感 = **Grok Video 1.5** 云 bulk；**肉戏/bare/高难** = **5090 MiniMax H3**；对白讲话镜锁 FRW LTX 2.3。矩阵见 [weapon-lane-matrix.md](weapon-lane-matrix.md)。非成人可纯 `grok_primary`。 |
...
| **H3 效果最大化（P0 · 2026-08-04）** | **模式**：**I2V**=有 still 的主角/肉戏/反应/续镜默认；**R2V**=高能量、换构图、对白大嘴 CU、状态链；**T2V**=仅无脸 env/bridge（**禁锁脸**）。**高动**=狠 prompt+动作/bare 状态 still（勿改 T2V 求动）。**续镜**=批准末帧→下一镜 I2V。**对白注入**=`audio_cues` 必进 prompt（与自定义 `receipts/prompts/*.i2v.txt` **合并**，禁被覆盖吃掉）。**运维**=换模式前 `comfy free-memory --confirm`；capacity VRAM≥24GiB；5090 独占串行。见 [weapon-lane-matrix](weapon-lane-matrix.md)·[h3-max-effect](lessons-2026-08-04-h3-max-effect.md) |
| **Motion Prompt Spine（P0 · 2026-08-04）** | 动向生成（**Grok I2V + H3**）必须携带电影核：`dramatic_function` → `want_beat`（自 `director_intent.protagonist_want/theme`）→ action/motion/visible_change → camera_prompt → 对白/foley。`build_shot_intent` 输出 `motion_tier`/`want_beat`/`has_action_core` 等核字段。`aifilm h3 run` / `media-queue` 对空核 prompt **fail closed**（`MOTION_CORE_*`）。代码：`scripts/motion_prompt_spine.py`。见 [weapon-lane-matrix](weapon-lane-matrix.md) |
| **Motion Core P1/P2（2026-08-04）** | **Variety 硬门**：`bulk-preflight` + `h3 run --register` 默认跑 `variety-precheck` fail-closed（逃生 `AIFILM_SKIP_VARIETY_PREFLIGHT=1`）。**DF 动门**：`i2v_motion_gate.motion_tier_for_shot` — soft DF（reaction/afterglow/insert…）mean≥**10**；medium（bare 后 afterglow 恢复）≥**16**；normal≥**18**；meat/high≥**20**；act/climax 永不因 soft DF 降档。**Continue handoff**：H3 成片写 `receipts/continue-handoff/<id>.json`+`_end.png`。**Closeout 电影核审计**（advisory）：`receipts/film-core-closeout.json`（DF/want/dialogue spine）。测：`tests/test_motion_core_p1.py` |
...
| I2V profile | 环境默认 `grok_primary`；**成人 max 片级自动 hybrid 行为**（Grok bulk + H3 meat）；显式 `AIFILM_I2V_PROFILE=hybrid_h3` 或 `h3.enabled`；退出：`h3.enabled=false` / soft heat；旧项目 `ltx23_primary` |
