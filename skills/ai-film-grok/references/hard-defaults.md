...
| **对白原音 IRON（P0 · 2026-08-05 · 补丁 ltx23_adult 2026-08-05）** | **有声镜生成源** = Grok Imagine Video · **5090 H3** · **opt-in FRW LTX 2.3 `img2video-audio`**（`AIFILM_I2V_PROFILE=ltx23_adult` 或 `motion_lanes.dialogue=frw_ltx23` / `allow_ltx_dialogue`）。路由：**restricted/bare/肉戏对白 → H3 hard**；**safe 对白 → LTX（opt-in）或 Grok**；`h3_primary` 全片 H3。成片混音 **`prefer_native`**。**冻结** 后期对嘴（LatentSync/MuseTalk/InfiniteTalk/FRW lipsync）。Edge 仅字幕时钟/ADR。**禁止** genre=adult 静默切 ltx23 全片 primary；肉戏永不静默改 LTX。代码：`production_router` `cloud_ltx23_audio` · `film_spec_profile.ltx23_adult`。 |
...
| **多 agent 共用 5090 禁抢闲占满（P0 · 2026-08-06 · 不准再犯）** | **`free-first` 不 cancel ≠ 可 until-empty 抢空档**。多 agent/多会话：**默认禁止** `h3 cycle --until-empty --execute` 与自动 restart supervisor。submit 前 Comfy `running+pending=0` 且 capacity ready；忙则 **零 submit**，回报 PARTIAL。用户点名「独占/今夜只烧」才允许排水长驻；喊「占满/别人要用」→ **立刻杀本片 drain + neuter supervisor**。记忆 [multi-agent-gpu-no-hog](../memory/2026-08-06-multi-agent-gpu-no-hog.md)。 |
...
| **时长目标诚实（P0 · 2026-08-06 Q4.1）** | `target_duration` vs `sum(duration_sec)`：缺口 **>12% soft / >20% hard**（`plan/duration_target` · bulk-preflight）；H3 默认单镜 ~**5.2s** → 5 分片建议 **≥58 镜** 或 FLF 加长。禁默默交付 −29% 短板当满目标。机读 `receipts/duration-target.json`。逃生 `AIFILM_SKIP_DURATION_TARGET=1`。见 [h3-native-ship-review](../memory/2026-08-06-h3-native-ship-review-lessons.md) |
