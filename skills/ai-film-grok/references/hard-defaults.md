...
| **Fill-Idle 挑战（P0 · 2026-08-04 定策）** | **Grok 铺 soft baseline**；**restricted 主轨 H3**。5090：**P0 meat/对白/续链 → P1 弱镜 → P2 空闲填空挑战 Grok**（能烧就烧，**禁**抢 P0）。**R2V=能量位**非全片默认。PK：`select-shortlist` 机读建议 + **人 promote**（禁 mean 静默换片）。**P2 排序**=mean 最低优先；final **不**阻塞于 P2 / 高光不强制挑战；跨集胜率不自动。见 [weapon-lane-matrix](weapon-lane-matrix.md) · [memory](../memory/2026-08-04-h3-fill-idle-challenge.md) |
