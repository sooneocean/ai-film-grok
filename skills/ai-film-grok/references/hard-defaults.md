...
| **画面抗重复·抗无聊（P0 · 2026-07-29 · 后面不要再犯）** | **门禁绿 ≠ 好看**。相邻 act/climax **禁止**同一 `dsl.motion` 主句；每镜换一维运动（轴/节奏/部位/轻镜头动）。`shot_size`≡`dsl.camera.shot_size` 真变；禁 insert 塞满头肩套话；肉戏块 ≥1 真 L4 定器 + ≥1 反应近景。主戏 **片上 ≥4.5–6s**，禁 VO-stretch 压成 3s 幻灯片。contact 相邻须一句话能说出不同。优先级：尺度弧 ＞ 抗无聊 ＞ mean 数字。见 [shot-variety-anti-boring](lessons-2026-07-29-shot-variety-anti-boring.md) · [memory](../memory/2026-07-29-shot-variety-anti-boring.md) |
...
| **收尾门禁 IRON（P0 · 2026-07-29 · 后面不要再犯）** | **有 plate ≠ 收尾完。** heat：S 分仍看 `codes`（`SEX_BOTH_UNDRESS_UNSTATED` → 写 `partner_wardrobe_state`）。adult sensory：act/climax `sex_sfx` 事件 + `mix_report.artifacts` 三轨 hash + AV≥90。改 film-spec 立刻刷 `truth_contract.contract_sha256`。pilot 用 `approved_by=user` + 白名单原话。still `frame_chain_seed`≠approved。字幕钟=真 concat（`film_timeline.shot_starts`）；cue 不跨 hard 切。改 final：**删** `out/quality-report.json` + **重绑** narrative-evidence。`register-final` post-engine=post_owner。motion `--rows`=JSON **文件路径**。链：review-final→post-audit `delivery_ready`→export-desktop。简化 final 须 PARTIAL honest_limits。见 [closeout-gates-chaebol](lessons-2026-07-29-closeout-gates-chaebol.md) |
