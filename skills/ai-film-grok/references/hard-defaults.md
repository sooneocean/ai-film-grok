...
| **Gate-auto 机读过闸（P0 · 2026-08-04 · opt 2.39.11）** | **禁止** agent 反复让用户手点闸。`aifilm gate-auto --root`：mean → i2v-final → sex_sfx → five_track → 单 take promote → true_video → variety → cinematic → `gate-auto.json` + **`machine-ready.json`**。**fast_path**：三收据已绿则不重测（`--force` 才重算）。空片 i2v soft。advance/dispatch/ship-prep/closeout/export 全链路机读。**永不**自动绿：pilot、多 take PK、review-final、付费 ack。逃生 `AIFILM_SKIP_GATE_AUTO=1`。 |
