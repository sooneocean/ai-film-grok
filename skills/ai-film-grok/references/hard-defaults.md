...
| **H3 官方 prompt 方言 · auto（P0 · 2026-08-07 · O3 canary）** | 编译器 `scripts/media/h3_official_prompt.py`（vendor `references/vendor/minimax-h3/`）。**默认 `auto`**：对白镜 → official（`<d>[Mandarin]` + 三字段）；**high motion → legacy** 时间轴（canary mean 20.7>18.6）；其余 official。强制：`AIFILM_H3_PROMPT_DIALECT=official\|legacy` 或 `dsl.prompt_format`。回执 `*.h3.official.txt`。`non_diegetic_music` 默认 N/A。证据 `skills/ai-film-grok/artifacts/2026-08-07-h3-official-ab-canary.json` · 计划 [import todo](../../docs/plans/2026-08-07-h3-official-prompt-import-todoplan.md)。 |
