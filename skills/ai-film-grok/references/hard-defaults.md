...
| **定装一装 + 嘴动片长 + 外片锁 GPU 逃逸（P0 · 2026-08-07 · gen2 · 不准再犯）** | **一装**：交付抽帧须 **同一 still 家族**（白束腰+红袖 vs 红腰马甲 vs 裸床单 **不可混** 当「没脱」）；I2V `--still` 定装 + prompt NEVER undress；禁乱 continue 代装。**嘴动**：对白/用户要能动 → **禁** hero Ken Burns/zoompan 死脸；须 H3/Grok **真 I2V** 张合。**片长**：用户点 ≥1min 则镜×秒够，禁 12s 糊弄。**外片锁**：他 root `gpu-owner-lock` ACTIVE → **零抢 5090、禁杀外 guardian**；可 **Grok 云端 I2V** 保交付，H3 等 RELEASED。剧情单集 **3–5 beat**。记忆 [one-outfit-mouth-min60](../memory/2026-08-07-one-outfit-mouth-min60-gpu-escape.md)。 |
