...
| **H3 官方 prompt 方言 · auto（P0 · 2026-08-07 · R5）** | 编译器 `h3_official_prompt`（HF base/ref GUIDE）。**默认 `auto`**：对白/R2V/多 ref→official；**high→legacy**（O3 mean；opt-in `AIFILM_H3_HIGH_MOTION_OFFICIAL=1`）。强制 `AIFILM_H3_PROMPT_DIALECT=official\|legacy`。plan 写 `prompt_preview` + `*.h3.preview.txt`；combo R5 official families。T2VA 禁 Picture1；official 禁 2V stage；validate fail-closed（`AIFILM_H3_OFFICIAL_SOFT=1` 逃）。[optimize todo](../../docs/plans/2026-08-07-h3-official-prompt-optimize-todoplan.md)。 |
