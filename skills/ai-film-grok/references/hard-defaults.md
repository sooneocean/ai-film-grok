...
| **Gate-auto 机读过闸（P0 · 2026-08-04 · deep 2.39.10）** | **禁止** agent 反复让用户手点闸。`aifilm gate-auto --root` 自动：mean → i2v-final → sex_sfx → five_track → 单 take promote → true_video → variety → cinematic → `receipts/gate-auto.json`。**advance/autopilot** 可本地执行；**dispatch** clips 齐推 gate-auto；**ship-prep** 收尾跑 gate-auto；**closeout/export** 红时自动 gate-auto。**永不**自动绿：pilot、多 take PK、review-final、付费 ack。逃生 `AIFILM_SKIP_GATE_AUTO=1`。代码：`scripts/gate_auto.py` · `advance.py` · `dispatch.py`。 |
