...
| **Gate-auto 机读过闸（P0 · opt 2.39.12）** | 机读阶梯 + **编排提速**：gate 收据 state-hash 只认 ok（不因 `at`  thrash）；advance 开 state cache；fast_path 绿则不重写；context max 2 refs / 4kB。`aifilm gate-auto`；`--force` 才重测。永不自动绿 pilot/PK/review-final。逃生 `AIFILM_SKIP_GATE_AUTO=1`。 |
