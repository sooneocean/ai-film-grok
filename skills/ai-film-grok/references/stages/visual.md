...
- **武器路由（P0 · 2026-08-05 h3_primary）**：**有 5090 推荐 `AIFILM_I2V_PROFILE=h3_primary`** → 全镜本地 H3 主产线（时间换无限产能；setup/meat/对白/env 均 comfy-h3；云 bulk 默认硬拦）。场景：I2V / FLF / R2V / T2V 见 [weapon-lane](../weapon-lane-matrix.md)。兼容 **`hybrid_h3`**：setup→Grok；肉戏→H3；对白→FRW LTX。成人 max 仍可 auto→hybrid；有 5090 请显式 `h3_primary`。逃生 `AIFILM_ALLOW_CLOUD_RESTRICTED=1`。dispatch 优先 `h3-run-next`。
