...
- **Fill-Idle（2.37.12）**：Grok 铺 soft；restricted 主轨 H3；`h3 next` / `h3 run-next --execute [--max N]`（P2=pilot；非 daemon）。climax/对白CU dual I2V→R2V。成片前 `ship-prep` 看 `human_pk_required`；人 `pk-compare`+`select-shortlist --promote` / `pk-ledger --append`。final 不等 P2 填完。见 [fill-idle memory](../../memory/2026-08-04-h3-fill-idle-challenge.md)。
