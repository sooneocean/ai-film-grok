...
- **H3 效果最大化（2026-08-04 · 2.37.2 自动 mode）**：**I2V** 锁脸默认；**R2V** 高能量·对白大嘴 CU；**T2V** 仅无脸 env。`h3 list|plan` 调 `resolve_h3_mode` 写 `mode`/`command`/`alt_mode`。续镜=末帧→I2V。换模式前 `comfy free-memory --confirm`。见 [h3-max-effect](../lessons-2026-08-04-h3-max-effect.md)·[weapon-lane-matrix](../weapon-lane-matrix.md)。
