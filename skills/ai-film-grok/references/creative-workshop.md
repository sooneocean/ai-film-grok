...
验证会从当前 Graph + Brief 重新编译并比较确定性 payload，因此即使有人重算收据自哈希，内容偏离
源投影仍会被拒绝；镜头 ID 必须非空且唯一。发现问题时回到故事图或对应 Bible 修改、按既有锁定与
审批规则重新投影，不得直接修改收据。

`apply` 写入的每个 `shot.creative` 都保留 `workshop_packet_sha256`；因此后续的 film-spec 和
Shot Package 可以回溯到确切的已验证导演包。
