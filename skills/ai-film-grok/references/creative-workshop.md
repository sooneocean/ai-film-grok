...
- `drama-graph.json`：故事、表演和镜头的唯一真相；workshop 只读取。
...
5. `export` 输出 Grok、FRW/Seedance 或通用的文本包；绝不提交请求。

默认验证仅报告兼容性警告；`--strict` 把它们升级为阻断。发现问题时回到故事图或对应 Bible 修改、按既有锁定与审批规则重新投影，不得直接修改收据。
