...
| Local I2V / T2V / R2V | `image-to-video`, `text-to-video`, `reference-to-video` | `minimax-h3-*-pilot` | **Film-lane open** (`verified` + `production_promoted`, `comfy-h3`); `aifilm h3 plan\|run` (no `--allow-experimental`); prefer_native; bulk still needs user pilot approval; Wan 2.2 local I2V stays retired |
| Local env T2V | `text-to-video`, `env-plate` | `minimax-h3-t2v-pilot` | 9:16 film-lane; native stereo when usable; do not treat as face-lock |
